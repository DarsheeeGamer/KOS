...
from .kscript import run_kscript_file
...
class KaedeShell(cmd.Cmd):
    ...
    def __init__(self, filesystem, kapp_manager, user_system):
        super().__init__()
        self.fs = filesystem
        self.km = kapp_manager
        self.us = user_system
    ...
    def do_ls(self, arg):
        ...
        try:
            ...
            if long_format:
                ...
                for file in files:
                    inode = self.fs.get_inode_by_path(file)
                    table.add_row(
                        str(inode.permissions),
                        f"{inode.uid}:{inode.gid}",
                        str(inode.size),
                        datetime.fromtimestamp(inode.modified_at).strftime('%Y-%m-%d %H:%M'),
                        file
                    )
    ...
    def do_kapp(self, arg):
        """KOS Package Manager (KPM)
        Usage: kapp <command> [options]
        Commands:
        ...
        try:
            ...
            if command == "update":
                ...
                name = args[1]
                url = args[2] if len(args) > 2 else f"https://{name}.kos-repo.org"
                if self.km.repo_config.add_repository(name, url):
