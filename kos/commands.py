...
from .package_manager import PackageManager  # Updated import to use new class name
...
class KaedeShell(cmd.Cmd):
    ...
    def __init__(self, filesystem: FileSystem, kpm_manager: PackageManager, user_system: UserSystem, process_manager=None):
        super().__init__()
        self.fs = filesystem
        self.km = kpm_manager 
        self.us = user_system
        ...
        self.reboot_function = None  # Function to be set in main.py for rebooting the system
    ...
    def do_kssp(self, arg):
        """Kaede Secure Shell Protocol (KSSP) client for remote access
        ...
        try:
            ...
            # Get the command to run, if specified
            command = ' '.join(args) if args else None
            
            # Set up SSL secure connection
            print(f"Connecting to {host}:{port}...")
            
            # Get password if not using key authentication
            password = None
            if not keyfile:
                password = getpass.getpass("Password: ")
            
            try:
                # Import KSSP client to establish connection
                from .kssp import KSSPClient, AuthenticationError, ProtocolError
                
                # Create client and connect
                client = KSSPClient()
                
                if verbose:
                    print("Establishing KSSP connection...")
                ...
                if client.connect(host, port):
                    ...
                    # Try to authenticate with key or password
                    auth_success = False
                    if keyfile:
                        auth_success = client.authenticate(user or self.us.current_user, key_path=keyfile)
                    else:
                        auth_success = client.authenticate(user or self.us.current_user, password=password)
                        
                    if auth_success:
                        if verbose:
                            print("Authentication successful")
                            
                        # If command specified, run it remotely
                        if command:
                            if verbose:
                                print(f"Executing remote command: {command}")
                                
                            exit_code, output = client.execute_command(command)
                            print(output)
                ...
                print("Error: KSSP module not available")
    ...
    def do_kscp(self, arg):
        """Kaede Secure Shell Protocol file transfer
        ...
        try:
            ...
            # Determine if source or target is remote
            is_source_remote = '@' in source and ':' in source
            is_target_remote = '@' in target and ':' in target
            
            if is_source_remote and is_target_remote:
                print("Error: Both source and target cannot be remote")
                return
                
            if not is_source_remote and not is_target_remote:
                print("Error: Either source or target must be remote")
                return
                
            # Import KSSP client for file transfer
            try:
                from .kssp import KSSPClient, AuthenticationError, ProtocolError
                ...
                user_host, remote_path = remote_arg.split(':', 1)
                ...
                if client.connect(host, port):
                    ...
                    auth_success = False
                    if keyfile:
                        auth_success = client.authenticate(user, key_path=keyfile)
                    else:
                        auth_success = client.authenticate(user, password=password)
                        
                    if auth_success:
                        if is_upload:
                            print(f"Uploading {local_path} to {user}@{host}:{remote_path}")
                            client.upload_file(local_path, remote_path, recursive)
                        else:
                            print(f"Downloading {user}@{host}:{remote_path} to {local_path}")
                            client.download_file(remote_path, local_path, recursive)
    ...
    def do_reboot(self, arg):
        ...
        This command will cleanly shutdown and restart the operating system.
        All filesystem changes will be saved to disk before reboot.
        """
        if hasattr(self, 'reboot_function') and callable(self.reboot_function):
            print("Rebooting KOS...")
            # First clean up resources
            self._cleanup_resources()
            # Then call the reboot function
            self.reboot_function()
        else:
            print("Reboot functionality not available")
            
    def _cleanup_resources(self):
        """Clean up resources before exit or reboot"""
        try:
            # Save all filesystem changes
            if hasattr(self, 'fs') and self.fs is not None:
                # Check for either _save_filesystem or save_filesystem method
                if hasattr(self.fs, '_save_filesystem'):
                    self.fs._save_filesystem()
                elif hasattr(self.fs, 'save_filesystem'):
                    self.fs.save_filesystem()
                
            # Save user data
            if hasattr(self, 'us') and self.us is not None and hasattr(self.us, 'save_users'):
                self.us.save_users()
                
            print("All system data saved to encrypted virtual disk")
        except Exception as e:
    ...
    def do_shutdown(self, arg):
        ...
        This command will cleanly shutdown the operating system.
        All filesystem changes will be saved to disk.
        """
