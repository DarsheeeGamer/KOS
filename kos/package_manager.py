    ...
    def _load_packages(self):
        ...
        try:
            ...
            for name, pkg_data in data["registry"].items():
                ...
                if pkg.installed:
                    pkg.install_date = data["installed"][name].get(
                        "installed_at")
    ...
    def install(self, package_name: str, version: str = "latest") -> bool:
        ...
        try:
            ...
            for repo_name, repo in self.repo_config.repos[
                ...
                if not repo["enabled"]:
                    print(
                        f"DEBUG: Repository '{repo_name}' is disabled, skipping"
                    )
                    continue

                print(
                    f"DEBUG: Processing repository: {repo_name}, URL: {repo['url']}, Enabled: {repo['enabled']}"
                )
                try:
                    ...
                    print(
                        f"DEBUG: Response status code (index.json): {response.status_code}"
                    )
                    if response.status_code == 200:
                        repo_data = response.json()
                        print(
                            f"DEBUG: index.json content:\n{json.dumps(repo_data, indent=2)}"
                        )
                        if package_name in repo_data.get("packages", {}):
                            ...
                            print(
                                f"DEBUG: Package '{package_name}' data from index.json:\n{json.dumps(package_data, indent=2)}"
                            )
                            ...
                            for file_name in package_files:
                                ...
                                print(
                                    f"DEBUG: Downloading from URL: '{raw_file_url}' to '{dst}'"
                                ...
                                try:
                                    ...
                                    print(
                                        f"DEBUG: Downloaded and saved to '{dst}'"
                                    )  # DEBUG PRINT - DOWNLOAD SUCCESS

                                except requests.exceptions.RequestException as download_error:
                                    print(
                                        f"DEBUG: Error downloading '{raw_file_url}': {download_error}"
                            ...
                            pkg = Package(
                                ...
                                install_date=
                                None,  # Removed install_date assignment
                                size=
                                0,  # Size not calculated due to checksum skip
                                checksum=
                                "FAKE_CHECKSUM_DISABLED",  # Indicate checksum is disabled
                                installed=True)
                            ...
                            print(
                                f"DEBUG: Package '{package_name}' not found in index.json from {repo['url']}"
                            )
                    else:
                        print(
                            f"DEBUG: Failed to fetch index.json from {repo['url']}: {response.status_code}"
                        )
                    ...
                    print(f"Error during package file download: {e}"
    ...
    def run_program(self, command: str, args: list = None) -> bool:
        ...
        try:
            ...
            for p in self.package_db.packages.values():
                if p.installed and (p.name == command
                                    or command in p.cli_aliases):
                    pkg = p
                    break
            ...
            if pkg.cli_function:
                cmd = [
                    'python3', '-c',
                    f"import {os.path.splitext(pkg.entry_point)[0]}; {pkg.cli_function}()"
                ]
            else:
                # --- REVERTING TO DIRECT SCRIPT EXECUTION BUT WITH PATH APPEND SIMULATION ---
                cmd = [
                    'python3', '-c',
                    f"import sys, os; sys.path.append('{app_dir}'); from calculator import cli_app; cli_app()"
                ]  # Modified cmd with explicit import and function call
