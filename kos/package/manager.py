...
class Package:
    ...
    def to_dict(self) -> Dict:
        return {
            ...
            'install_date':
            self.install_date.strftime("%Y-%m-%d %H:%M:%S") if hasattr(self.install_date, 'strftime') else self.install_date,
            'size':
            self.size,
            'checksum':
            self.checksum,
    ...
    def from_dict(data: Dict) -> 'Package':
        ...
        if data.get('install_date'):
            try:
                # Try to parse as datetime if it's a string with format %Y-%m-%d %H:%M:%S
                if isinstance(data['install_date'], str):
                    try:
                        install_date = datetime.strptime(data['install_date'], "%Y-%m-%d %H:%M:%S")
                    except ValueError:
                        # Fallback to ISO format for backward compatibility
                        try:
                            install_date = datetime.fromisoformat(data['install_date'])
                        except ValueError:
                            # If all parsing fails, just use the string directly
                            install_date = data['install_date']
                else:
                    # If it's not a string, use it as is
                    install_date = data['install_date']
            except Exception as e:
                # In case of any error, use the string value directly
                install_date = data['install_date']
        return Package(
            name=data['name'],
    ...
    def get_install_info(self) -> str:
        ...
        if hasattr(self.install_date, 'strftime'):
            return f"Installed: {self.install_date.strftime('%Y-%m-%d %H:%M:%S')}"
        else:
            return f"Installed: {self.install_date}"
